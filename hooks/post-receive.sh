...
git --git-dir="${PRIVATE_DIR}sahil.me/.git/" fetch
git --git-dir="${PRIVATE_DIR}sahil.me/.git/" --work-tree="${PRIVATE_DIR}sahil.me/" merge origin/master

# Copy files
for f in $(ls -a1 "${PRIVATE_DIR}sahil.me/")
do
	[ -f "${PRIVATE_DIR}sahil.me/${f}" ] && echo "Copying ${f}..."
